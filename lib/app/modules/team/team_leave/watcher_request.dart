...
class WatcherRequest extends StatefulWidget {
  @override
  _ReoprtPageState createState() => _ReoprtPageState();
}

class _ReoprtPageState extends State<WatcherRequest>
    with AutomaticKeepAliveClientMixin<WatcherRequest> {
  ...
  Future<TeamLeaveReport> monthlyTeamEmployeeLeave(
    ...
    if (response.result!.watchers!.isEmpty) {
      setState(() {
        isEmpty = true;
      });
    } else {
                          ...
                          Container(
                            child: ListView.builder(
                                ...
                                itemCount: items == null
                                    ? 0
                                    : items.result?.watchers?.length,
                                itemBuilder: (context, index) {
                                  String sRequestID = items.result!.watchers![index].id.toString();
                                  String createOn = TimeHandler.getDateFromUnix(
                                      items.result!.watchers![index].createdOn
                                          .toString());
                                  String createdByName =
                                  items.result!.watchers![index].createdByName
                                      .toString();
                                  String modifiedByName =
                                  items.result!.watchers![index].modifiedByName
                                      .toString();
                                  int? leaveStatusId = items
                                      .result!.watchers![index].leaveStatusId;
                                  int? iCancellationStatusId = items
                                      .result!.watchers![index].cancellationStatusId;
                                  String numberOfDays = items
                                      .result!.watchers![index].numberOfDays
                                      .toString();
                                  String startDate = TimeHandler.convertDate(
                                      TimeHandler.getDateFromUnix(items
                                          .result!.watchers![index].startDate
                                          .toString()
                                          .substring(0, 10)));
                                  String endDate = TimeHandler.convertDate(
                                      TimeHandler.getDateFromUnix(items
                                          .result!.watchers![index].endDate
                                          .toString()
                                          .substring(0, 10)));
                                  String leaveTypeName = items
                                      .result!.watchers![index].leaveTypeName
                                      .toString();
                                  String assignedToName = items
                                      .result!.watchers![index].assignedToName
                                      .toString();
                                  bool? isPending =
                                      items.result!.watchers![index].pending;
                                  int? cancellationRequestedBy = items.result!
                                      .watchers![index].cancellationRequestedBy;
                                  String reason =
                                  items.result!.watchers![index].reason
                                      .toString();
                                  return GestureDetector(
                                    ...
                                    child:
                                    TeamLeaveRequests.teamLeaveWatchCard(
                                        context,
                                        isPending,
                                        createdByName,
                                        "LEREQ" + sRequestID,
                                        leaveStatusId,
                                        iCancellationStatusId,
                                        numberOfDays,
                                        startDate + " - " + endDate,
                                        createOn,
                                        leaveTypeName,
