...
import '../../../../../widget/cards/team_ar.dart';
import '../../../../../widget/progress_dialog.dart';

class TeamArHistory extends StatefulWidget {
  @override
  _ReoprtPageState createState() => _ReoprtPageState();
}

class _ReoprtPageState extends State<TeamArHistory>
    with AutomaticKeepAliveClientMixin<TeamArHistory> {
  String sMonthName = "00";
  int daysInMonth = 0;
  late DateTime currentDate;
  bool loading = false;
  bool isEmpty = false;
  Future<TeamArReport>? _myFuture;
  ...
  Future<TeamArReport> monthlyTeamEmployeeLeave(
      String sYear, String sMonth) async {
    TeamArReport response = await ApiHandler().getTeamArReport(sYear, sMonth);
          ...
          child: Container(
            child: Column(
              ...
              children: [
                ...
                FutureBuilder<TeamArReport>(
                  future: _myFuture,
                  builder: (context, data) {
                    if (data.hasError) {
                      return Center(child: Text("${data.error}"));
                    } else if (data.hasData) {
                      var items = data.data as TeamArReport;
                      return Column(
                        children: [
                          Container(
                            child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return GestureDetector(
                                    ...
                                    child:
                                    TeamArRequests.teamArHistoryRequestCard(
                                        context,
                                        createdByName,
                                        sRequestID,
                                        attendanceTypeName,
                                        outTime,sInTime+"-"+sOutTime,comment,statusName),
                                  );
                                }),
                          ),
                        ],
